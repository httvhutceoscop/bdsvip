...
//	Call Step Functions
    rechargeStep1:function(other, hack, notshow){
        ...
        //    function(json){
        //        if(true || json.err == 0){
        //            if(true || json.customer.is_active_mail > 0){
        //                //if(other){
        ...
        //                //}
        //            }else{
        //                alert('Quý khách vui lòng kích hoạt Email trước khi nạp tiền vào Ví Rento');
    ...
    rechargeStep3_online:function(){
        shop.show_overlay_popup('gold-recharge3-online','Nạp vào Ví Rento',
            shop.gold.theme.rechargeStep3_online('gold-recharge3-online','Nạp vào Ví Rento'),
            {
                background: {
                    'background-color' : 'transparent'
                },
    ...
    rechargeStep3_card:function(){
        shop.show_overlay_popup('gold-recharge3-card','Nạp từ thẻ cào điện thoại',
            shop.gold.theme.rechargeStep3_card('gold-recharge3-card','Nạp từ thẻ cào điện thoại'),
            {
                background: {
                    'background-color' : 'transparent'
                },
                ...
                release:function(){
                    jQuery('#gold_recharge_seri').focus();
                    shop.enter('#gold_recharge_seri', shop.gold.doRechargeStep3_card);
...
//	Process steps
    ...
    doRechargeStep3_online:function(){
        ...
        if(money == ''){
            ...
            shop.redirect(BASE_URL+'nap-gold.html?gold='+money);
        }
    },
    doRechargeStep3_card:function(){
        if(!shop._store.variable['recharge_gold_start']){
            var frm = document.goldRecharge3Form_card, code = '', card_type = '', seri = '',
                email = (shop.gold.conf.type == 'self') ? shop.gold.conf.customer.email : shop.gold.conf.re_email;
            if(frm.gold_recharge_code){
                ...
                //card_type = 'shpcard';
                seri = shop.util_trim(frm.gold_recharge_seri.value);
                code = shop.util_trim(frm.gold_recharge_code.value);
                if(seri == ''){
                    shop.error.set('#'+frm.gold_recharge_seri.id, 'Chưa nhập số seri thẻ cào', 340, '.goldRecharge3_card'); return;
                //}else if(seri.length > 9){
                //    shop.error.set('#'+frm.gold_recharge_seri.id, 'Số seri không hợp lệ', 340, '.goldRecharge3_card'); return;
                }else{
                    shop.error.close('#'+frm.gold_recharge_seri.id, '.goldRecharge3_card');
                    if(code == ''){
                ...
                shop.ajax_popup('act=gold&code=recharge',"POST",{code_card:code, card_type: card_type, card_seri: seri},
                    function(j){
                        shop._store.variable['recharge_gold_start'] = false;
                        if(j.err == 0 && j.msg == 'success')	{
                            ...
                            switch(j.msg){
                                case 'not_connect':
                                    j.msg = 'Không kết nối được với sohapay';
                                    break;
                                case 'not_customer':
                                    j.msg = 'Hiện tại bạn đang không đăng nhập.<br />Vui lòng tắt cửa sổ, load lại trang web và thực hiện lại';
                                    break;
                                case 'code_invalid':
                                case 'invalid_card':
                                case 'error':
...
//	Theme steps
    theme:{
        rechargeStep1:function(id, title){
            ...
            ('<div class="gold-title"><label for="radio_self">Nạp tiền vào Ví Rento(<span>'+shop.gold.conf.customer.email+'</span>)</label></div>')
            ('<ul>')
            ('<li>- Bạn đang có: <b class="ff5a00">'+shop.numberFormat(shop.gold.conf.customer.gold,0,'','.')+' đ</b></li>')
            ('<li>- Nạp thêm tiền vào Ví Rento để có thể mua hàng dễ dàng chỉ trong vài nhấp chuột</li>')
            ('</ul>')
            ('</div>')
            ('<div class="c"></div>')
            ('</div>')
            ...
            ('<div class="fl"><label for="radio_other">Nạp hộ vào Ví Rento cho tài khoản khác</label></div>')
            ...
            ('</div>')
            ('<div>')
            ('<a href="javascript:void(0)" onclick="shop.gold.doRechargeStep1()" class="rt_regiter_button  mLeft10" id="fr"><span><span>TIẾP TỤC <b>&raquo;</b></span></span></a>')
            ('<div class="c"></div>')
            ('</div>')
            ('</form>')
            ('</div>')());
        ...
        rechargeStep2:function(id, title){
            var type = shop.gold.conf.pay_type,
                card = shop.join
                ('<div class="gold-content">')
                ('<div class="radioBox mTop5" style="float:left;"><input type="radio" value="card" id="radio_card_gold" name="radio_pay_gold"'+(type == 'card'?' checked':'')+' /></div>')
                ('<div class="gold-right" style="margin:0 0 20px 5px;float:left;">')
                ('<div class="gold-title"><label for="radio_card_gold" style="font-size:14px;cursor:pointer;font-weight:bold;height:24px;line-height: 24px;">Nạp từ thẻ cào điện thoại</label></div>')
                ...
                online = shop.join
                ('<div class="gold-content">')
                ('<div class="radioBox mTop5" style="float:left;">'+((shop.gold.conf.type != 'self') ? '' : '<input type="radio" value="online" id="radio_online_gold" name="radio_pay_gold"'+(type == 'online'?' checked':'')+' />')+'</div>')
                ('<div class="gold-right"  style="margin:0 0 20px 5px;float:left;">')
                ('<div class="gold-title"><label for="radio_online_gold" style="font-size:14px;cursor:pointer;font-weight:bold;height:24px;line-height: 24px;">Nạp từ thẻ Visa, Master Card, thẻ ATM, có Internet Banking</label></div>')
                ...
                ('<a class="fl" href="javascript:void(0)"><span id="agri"></span></a>')
                ...
                ('<div class="c"></div></div>')
                ('<div class="mTop5">')
                ('<a class="fl" href="javascript:void(0)"><span id="pgbank"></span></a>')
                ('<a class="fl" href="javascript:void(0)"><span id="gpbank"></span></a>')
                ('<a class="fl" href="javascript:void(0)"><span id="nama"></span></a>')
                ('<a class="fl" href="javascript:void(0)"><span id="sea"></span></a>')
                ('<a class="fl" href="javascript:void(0)"><span id="saigon"></span></a>')
                ('<div class="c"></div></div>')
                ('<div class="mTop5">')
                ('<a class="fl" href="javascript:void(0)"><span id="baca"></span></a>')
                ('<a class="fl" href="javascript:void(0)"><span id="vpbank"></span></a>')
                ('<a class="fl" href="javascript:void(0)"><span id="bidv"></span></a>')
                ('<div class="c"></div></div>')
                ('</div>')
                ('</div>')
                ('<div class="c"></div>')
                ('</div>')(),
                information = '';
            card_khuyenmai = shop.join
            ('<div class="gold-content" style="border-top:1px solid #ccc;padding:10px 0 0">')
            ('<div class="radioBox mTop5"><input type="radio" value="card" id="radio_card_gold" name="radio_pay_gold"'+(type == 'card'?' checked':'')+' /></div>')
            ('<div class="gold-right">')
            ('<div class="gold-title"><label for="radio_card_gold">Nạp vào Ví Rento từ thẻ cào khuyến mãi</label></div>')
            ('<div class="paymentText"></div>')
            ('<div class="bankPay">')
            ('<a class="fl" href="javascript:void(0)" style="height:44px;margin:13px 0 0"><span id="ico_muachung"></span></a>')
            ('<a class="fl" href="javascript:void(0)" style="height:70px"><span id="icon_sohapay"></span></a>')
            ('<div class="c"></div>')
            ('</div>')
            ('</div>')
            ('<div class="c"></div>')
            ...
            (information+'<div id="cError"></div>'+online+card)
            ('<div>')
            ('<a href="javascript:void(0)" onclick="shop.gold.doRechargeStep2()" class="rt_regiter_button  mLeft10" id="fr"><span><span>TIẾP TỤC <b>&raquo;</b></span></span></a>')
                //('<a href="javascript:void(0)" onclick="shop.gold.rechargeStep1()" class="rt_regiter_button  mLeft10" id="fr"><span><span><b>&laquo;</b> QUAY LẠI</span></span></a>')
            ('<div class="c"></div>')
            ('</div>')
            ('</form>')
            ('</div>')());
        ...
        rechargeStep3_online:function(id, title){
            ...
            ('<div class="fl mTop15"><a href="'+PAY_GATE_SITE+'" target="_blank" title="'+PAY_GATE_SITE_NAME+'">')
            ('<img src="style/images/logo/'+PAY_GATE_LOGO+'" height="97" width="196" border="0" />')
            ('</a></div>')
            ...
            ('<div class="mTop5">')
            ('<a class="fl" href="javascript:void(0)"><span id="pgbank"></span></a>')
            ('<a class="fl" href="javascript:void(0)"><span id="gbbank"></span></a>')
            ('<a class="fl" href="javascript:void(0)"><span id="nama"></span></a>')
            ('<a class="fl" href="javascript:void(0)"><span id="sea"></span></a>')
            ('<a class="fl" href="javascript:void(0)"><span id="saigon"></span></a>')
            ('<div class="c"></div></div>')
            ...
            ('</div>')
            ('<div class="mTop10">')
            ('<a href="javascript:void(0)" onclick="shop.gold.doRechargeStep3_online()" class="rt_regiter_button  mLeft10" id="fr"><span><span>TIẾP TỤC <b>&raquo;</b></span></span></a>')
            ('<a href="javascript:void(0)" onclick="shop.gold.rechargeStep2()" class="rt_regiter_button  mLeft10" id="fr"><span><span><b>&laquo;</b> QUAY LẠI</span></span></a>')
            ('<div class="c"></div>')
            ('</div>')
            ('</form>')
            ('</div>')());
        ...
        rechargeStep3_card:function(id, title){
            ...
            ('<div class="gold-content mTop10">')
            ('<div class="gold-title">')
            ('<div class="fl"><label>Số seri thẻ cào</label></div>')
            ('<div class="fl mLeft5">')
            ('<input type="text" id="gold_recharge_seri" name="gold_recharge_seri" value="" maxlength="30" />')
            ('<div style="color:#787878;font-size: 11px;padding-top:5px;text-align: justify;">Vui lòng nhập chính xác mã số seri in trên thẻ</div>')
            ('</div>')
            ('<div class="c"></div>')
            ('</div>')
            ('</div>')
            ...
            ('<a href="javascript:void(0)" onclick="shop.gold.doRechargeStep3_card()" class="rt_regiter_button  mLeft10" id="fr"><span><span>TIẾP TỤC <b>&raquo;</b></span></span></a>')
            ('<a href="javascript:void(0)" onclick="shop.gold.rechargeStep2()" class="rt_regiter_button  mLeft10" id="fr"><span><span><b>&laquo;</b> QUAY LẠI</span></span></a>')
            ('<div class="c"></div>')
            ('</div>')
            ('</form>')
